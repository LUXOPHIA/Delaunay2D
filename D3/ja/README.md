...
| `FindPoin( Pos_, Radius_ ) :TDelaPoin3D` | `Radius_` 以内の最近傍頂点。無ければ `nil`。 |
| `AddPoin( Pos_ ) :TDelaPoin3D` | 点の追加（Bowyer–Watson 法）。所属胞が既知ならオーバーロード `AddPoin( Pos_, Cell_ )` で検索を省けます。 |
| `DeletePoin( Poin_ ) :Boolean` | 頂点の削除（フリップ法・下記）。不正な入力（`nil`・無限遠頂点・他の図の頂点）は `False`。 |
...
begin
     ...
     P := D.FindPoin( TSingle3D.Create( 0, 0, 0 ), 1 );      // 半径 1 以内の最近傍頂点
...
- **追加** は Bowyer–Watson 法です。追加点を外接球に含む胞群（キャビティ）を再帰的な歩行で削除し、境界面ごとに新しい点へ張り直します。3D ではキャビティの双対が木にならず、再帰が削除済みの胞へ再突入し得るため、一時的なプレースホルダ胞が縫合待ちの「受け箱」として働きます。プレースホルダは再帰が終わればすべて消えます。
- **削除** はフリップ法です。頂点の星の耳を *元の* リンク全体に対して検定し（「耳の球が頂点を含み、他のリンク頂点を含まない」＝ 耳が最終形の胞そのもの）、2-3 / 3-2 フリップで確定していきます。凸包の近くでは組合せ的な環の優先順位を使い、そこで生じる体積ゼロのポケット対は 2-0 フリップで除去します。星が4胞まで縮んだら1胞に畳み込みます。フリップで畳み切れない退化配置（数%）は、点のインスタンスを保ったまま胞を作り直して解決するため、削除は常に正しいドロネー図を残します。
