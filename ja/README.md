...
- **ファイル保存** — `SaveToFile` / `LoadFromFile` が図を `*.lxtf`（2D）/ `*.lxtc`（3D）ファイルへ読み書きします。Radiance HDR 形式と同じく、UTF-8 テキストのヘッダ（固有の1行目とオプション行）・空行・バイナリデータという構成で、座標と接続構造の全てを含むため図がそのまま往復します。
