...
- **完全に動的** — 点はいつでも追加（Bowyer–Watson 法）・削除（フリップ法）できます。どの操作の後も、構造は常に正しいドロネー図です。
...
begin
     ...
     P := D.FindPoin( TSingle2D.Create( 0, 0 ), 10 );          // 半径 10 以内の最近傍点
...
begin
     ...
     P := D.FindPoin( TSingle3D.Create( 0, 0, 0 ), 1 );  // 半径 1 以内の最近傍点
